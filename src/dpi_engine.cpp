...
bool DPIEngine::initialize() {
    if (config_.num_load_balancers <= 0 || config_.fps_per_lb <= 0) {
        std::cerr << "[DPIEngine] Error: thread counts must be positive\n";
        return false;
    }
...
bool DPIEngine::processFile(const std::string& input_file,
    ...
    // Stop all threads. Each worker drains its queue before exiting.
    stop();
