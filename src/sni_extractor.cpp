...
std::optional<std::string> SNIExtractor::extract(const uint8_t* payload, size_t length) {
    ...
    if (offset + 4 + handshake_length > length) {
        return std::nullopt;
    }
    offset += 4;
...
std::vector<std::pair<uint16_t, std::string>> SNIExtractor::extractExtensions(
    ...
    size_t offset = 5 + 4;  // TLS record header + handshake header
    if (offset + 34 > length) return extensions;

    offset += 2;   // Client version
    offset += 32;  // Random

    if (offset >= length) return extensions;
    uint8_t session_id_length = payload[offset];
    offset += 1 + session_id_length;

    if (offset + 2 > length) return extensions;
    uint16_t cipher_suites_length = readUint16BE(payload + offset);
    offset += 2 + cipher_suites_length;
    ...
    size_t extensions_end = std::min(length, offset + static_cast<size_t>(extensions_length));
    while (offset + 4 <= extensions_end) {
        uint16_t extension_type = readUint16BE(payload + offset);
        uint16_t extension_length = readUint16BE(payload + offset + 2);
        offset += 4;
        ...
        std::string value;
        if (extension_type == EXTENSION_SNI && extension_length >= 5) {
            uint8_t sni_type = payload[offset + 2];
            uint16_t sni_length = readUint16BE(payload + offset + 3);
            if (sni_type == SNI_TYPE_HOSTNAME && sni_length <= extension_length - 5) {
                value.assign(reinterpret_cast<const char*>(payload + offset + 5), sni_length);
            }
        }

        extensions.emplace_back(extension_type, value);
        offset += extension_length;
    }
...
std::optional<std::string> QUICSNIExtractor::extract(const uint8_t* payload, size_t length) {
    ...
    // Best-effort fallback: some synthetic captures include an intact TLS
    // record inside the QUIC payload. Real QUIC needs full CRYPTO frame parsing.
    for (size_t i = 0; i + 9 < length; i++) {
        if (payload[i] == 0x16) {
            auto result = SNIExtractor::extract(payload + i, length - i);
