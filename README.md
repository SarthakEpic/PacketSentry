...
```powershell
.\build\Release\packetdpi.exe filter test_dpi.pcap filtered.pcap --block-domain www.google.com
```

Benchmark parser/classifier throughput:
...
- PCAP file parsing with Ethernet, IPv4, TCP, UDP, payload offsets, and timestamps.
- Application classification from TLS SNI, HTTP Host, DNS queries, and port fallback.
- Rule-based filtering by IP, domain, wildcard domain, application, and port.
- Deterministic offline output PCAP generation.
...
src/                     C++ implementation and CLIs
tests/                   Unit tests
demo/                    Demo commands, rules, and expected report notes
docs/                    Architecture and responsible-use notes
benchmarks/              Benchmark guidance
.github/workflows/       CI build and test workflow
```

## Current Scope and Limitations

PacketDPI 1.0 is an offline PCAP tool. It does not capture live traffic and does not decrypt TLS. Classification is based on visible metadata such as TLS SNI, HTTP Host headers, DNS queries, and ports. The repository also contains a compiled multi-stage threaded engine prototype; the public `packetdpi filter` command intentionally uses the deterministic offline pipeline so the first-run reviewer experience is reliable.
...
- [Security and Ethics](docs/SECURITY_ETHICS.md)

---

# Deep Packet Inspection System - Technical Walkthrough
